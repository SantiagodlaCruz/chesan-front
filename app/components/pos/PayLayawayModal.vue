  ...
  >
    <div class="space-y-6" v-if="ticket">
      ...
      <div class="p-4 bg-slate-100 dark:bg-white/5 rounded-xl border border-slate-200 dark:border-white/10">
         ...
         <h4 class="text-lg font-black text-slate-900 dark:text-white break-all leading-tight">{{ ticket.ticket_number }}</h4>
      ...
      <div class="bg-indigo-50 dark:bg-indigo-500/10 border border-indigo-100 dark:border-indigo-500/20 rounded-xl p-4">
        <div class="flex justify-between items-center mb-2">
           ...
           <span class="text-sm font-mono text-slate-800 dark:text-slate-200">${{ parseFloat(ticket.total).toFixed(2) }}</span>
        </div>
        <div class="flex justify-between items-center mb-3">
           <span class="text-xs font-bold text-slate-600 dark:text-slate-400">Anticipo Original:</span>
           <span class="text-sm font-mono text-slate-800 dark:text-slate-200">${{ parseFloat(ticket.received_amount).toFixed(2) }}</span>
        </div>
        <div class="h-px bg-indigo-200 dark:bg-indigo-500/20 mb-3"></div>
        <div class="flex justify-between items-center">
           <span class="text-sm font-black text-indigo-800 dark:text-indigo-400 uppercase tracking-widest">Saldo A Pagar:</span>
           <span class="text-2xl font-black text-indigo-600 dark:text-indigo-300">${{ parseFloat(ticket.balance).toFixed(2) }}</span>
        </div>
      </div>

      <!-- Items List -->
      <div>
         <h3 class="text-[10px] font-black tracking-widest uppercase text-slate-400 mb-3 ml-1">Productos Resguardados</h3>
